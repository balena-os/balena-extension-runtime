...
func Run(ctx context.Context, logger *slog.Logger, o Options) error {
	...
	// Not a verdict: the sweep retires records.
	if armed == "" {
	...
	if err != nil {
		// Bounded: pending rejects a spent boot budget.
		return err
	}
	if passed {
	...
	// The device still runs the rejected kernel.
	return reboot()
}
...
//
// A spent boot budget rejects the arm without a reboot: this boot already runs
// the kernel the restored arm names.
func pending(ctx context.Context, logger *slog.Logger, env *bootenv.Env,
	...
	if !published {
		// Claimed but unpublished is a defect, not a verdict.
	...
	// Read only here: the other rows tested nothing.
	if boots := trialCount(env); boots >= trialLimit {
		// Stage 2 stops offering the arm at the limit.
...
func forget(ctx context.Context, abi string) error {
	return withOperationLock(ctx, func() error {
		armCleared, err := bootenv.Forget([]string{abi})
		if err != nil {
			return err
		}
		if !armCleared {
			// A deploy re-armed meanwhile; its prestate stays.
			return override.RemoveKernel(abi)
		}
		return errors.Join(override.RemoveKernel(abi), override.RemoveHealthPrestate())
	})
}
...
func commit(ctx context.Context, logger *slog.Logger, abi string, slot bootenv.Slot) error {
	return withOperationLock(ctx, func() error {
		...
		if !wrote {
			// The new window's prestate is its baseline.
...
func restore(ctx context.Context, logger *slog.Logger, slot bootenv.Slot) error {
	return withOperationLock(ctx, func() error {
		...
		if !wrote {
			// A deploy armed one meanwhile; it owns its boot.
...
func reject(ctx context.Context, logger *slog.Logger, env *bootenv.Env,
	...
	return withOperationLock(ctx, func() error {
		...
		if !wrote {
			// The records stand; the new window keeps its prestate.
