...
func TestRun_StateMachine(t *testing.T) {
	tests := []struct {
		...
		{
			// Claimed with no published kernel is a defect.
...
func TestRun_AMovedArm(t *testing.T) {
	...
	t.Run("forgetting an unpublished arm leaves the new window its prestate", func(t *testing.T) {
		w := newWorld(t, bootenv.KeyABI+"="+abiX)
		w.publish(abiY)
		w.claims = []string{abiX, abiY}
		// The deploy lands after Run read the block.
		w.onClaim = func() {
			w.writeBlock(bootenv.KeyABI + "=" + abiY)
			w.writePrestate()
		}
		...
		require.NoError(t, w.run())
		assert.Equal(t, map[string]string{bootenv.KeyABI: abiY}, w.block())
		assert.True(t, w.prestateExists(), "activation wrote it when it armed the new window")
	})
...
func TestRun_CrashPoints(t *testing.T) {
	tests := []struct {
		...
		{
			// Written first, and a replay must not double it.
		...
		{
			// The next arm replaces a stale prestate.
