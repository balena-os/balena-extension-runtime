...
func TestForget(t *testing.T) {
	tests := []struct {
		...
		{
			// An unconditional clear would refresh the arm's budget.
...
func TestRestore(t *testing.T) {
	tests := []struct {
		...
		{
			// A deploy armed one; it owns the next boot.
...
func TestReject(t *testing.T) {
	tests := []struct {
		...
		{
			// The extension-rollback:217-223 window, as one write.
...
func TestHUPReject(t *testing.T) {
	tests := []struct {
		...
		{
			// Relaying would forget a proven ABI everywhere.
