...
// TestReadResponse_RefusesChunked pins that a chunked reply is refused rather
// than handed back with the chunk headers embedded in the body.
...
// TestReadResponse_RejectsNegativeContentLength pins that a negative length is
// refused. LimitReader would yield an empty body, turning a malformed response
...
// TestReadResponse_RejectsNonHTTPStatusLine pins that dialing something which
// is not an HTTP server fails here, not as a JSON decode error downstream.
...
// TestReadResponse_EOFDelimitedBody covers a reply with no Content-Length,
// whose body runs to connection close. The engine answers this way when it
...
func TestReadResponse_MatchesStdlibServer(t *testing.T) {
	for _, tc := range []struct {
		...
		{"no content", func(w http.ResponseWriter, r *http.Request) {
			// The DELETE endpoints answer 204 with no body.
