...
//
// The file is never unlinked. Unlinking would race a process that has already
// opened the old inode, leaving two holders locked against different inodes
// and neither aware of the other.
...
//
// Neither wait rejects a context that is already done: the select has two ready
// cases when the lock is free, and a non-blocking flock succeeds without
// consulting ctx. ctx is re-checked before fn, so a cancelled caller cannot
// reach it, rather than resting on every implementation short-circuiting on its
// own first engine call.
func WithOperationLock(ctx context.Context, fn func() error) error {
	...
	// Neither wait above rejects a context already done.
