...
// TestCleanup_GarbageSweep_RemovesOnlyOnStateError
...
func TestCleanup_GarbageSweep_RemovesOnlyOnStateError(t *testing.T) {
	stub := newEngineStub()
	const id = "in-flight-cncr"
	stub.Containers = []Container{
		{ID: id, State: "created", Labels: overlayLabels(nil)},
	}
