...
// volume and leaves its contents alone: a filled volume is a previous fill of
// the same image id, and rewriting it would truncate a kernel boot-by-abi may
// already link to.
func TestFabricate_Idempotent(t *testing.T) {
	...
	mountpoint := volumeMountpoint(t, name)
	written := filepath.Join(mountpoint, "written-after-fill")
	...
	assert.FileExists(t, written, "a filled volume must survive a second create")
